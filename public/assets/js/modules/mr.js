...
function getTaxSummary()
{
    ...
    $("#itemTable [id*='row_']").each(function(index, row) {
        row = $(row);
        let qty = Number(row.find('[name*="[quantity]"]').val());
...
$(document).on('change',"[name*='order_qty']",(e) => {
    ...
    let acceptedQuantity = $(e.target).closest('tr').find("[name*='quantity']");
...
/*qty on change*/
$(document).on('change',"[name*='quantity']",(e) => {
    console.log('hi');
    let tr = e.target.closest('tr');
    let qty = e.target;
    let dataIndex = $(e.target).closest('tr').attr('data-index');
...
function setTableCalculation() {
    ...
    $("#itemTable [id*='row_']").each(function (index, item) {
        ...
        let qty = $(item).find("[name*='[quantity]']").val() || 0;
        let rate = $(item).find("[name*='[rate]']").val() || 0;
        let itemValue = (Number(qty) * Number(rate)) || 0;
        console.log('teesstt------->>>', rowCount, qty, rate, itemValue);
    ...
    $("#itemTable [id*='row_']").each(function (index, item2) {
        ...
        let qty2 = $(item2).find("[name*='[quantity]']").val() || 0;
    ...
    $("#itemTable [id*='row_']").each(function (index, item3) {
        ...
        let qty3 = $(item3).find("[name*='[quantity]']").val() || 0;
    ...
    Promise.all(taxPromises).then(() => {
        ...
        $("#itemTable [id*='row_']").each(function (index, item4) {
            ...
            let qty4 = $(item4).find("[name*='[quantity]']").val() || 0;
        ...
        $("#itemTable [id*='row_']").each(function (index, item5) {
            ...
            let qty5 = $(item5).find("[name*='[quantity]']").val() || 0;
            ...
            let itemHeaderDisc = Number($(item5).find("[name*='[discount_amount_header]']").val()) || 0;
            let itemHeaderExp = 0;
            if(totalHeaderExp) {
                let itemPriceAterBothDis =  itemValue5 - itemDisc5 - itemHeaderDisc;
                itemHeaderExp =  itemPriceAterBothDis / totalAfterTax * totalHeaderExp;
                $(item5).find("[name*='[exp_amount_header]']").val(itemHeaderExp.toFixed(2));
            } else {
                $(item5).find("[name*='[exp_amount_header]']").val(itemHeaderExp.toFixed(2));
            }
        });
...
$(document).on('click', '.deleteItemDiscountRow', (e) => {
    ...
    if(!id) {
        ...
        $("#eachRowDiscountTable .display_discount_row").each(function(index,item){
            let disAmount = $(item).find("[name='item_d_amnt']").val();
...
$(document).on('click', '#add_new_item_dis', (e) => {
    ...
    let item_dis = 0;
    console.log('ertyuiop---------->>', new_item_dis_name, new_item_dis_perc, new_item_dis_value)
...
$(document).on('keyup', '#new_item_dis_perc', (e) => {
    ...
    let percAmount = 0;
    let totalCost = 0
    console.log('hi', rowCount,input, value);
    if (isNaN(value)) {
        input.val('');
        value = 0;
    }
