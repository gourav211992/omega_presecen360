    ...
    $("#itemTable [id*='row_']").each(function (index, item3) {
        ...
        if (price > 0 && itemId) {
            if (isTax) {
                ...
                const revisionNumber = params.get("revisionNumber");
            ...
            source: function (request, response) {
                $.ajax({
                    ...
                    data: {
                        ...
                        service_alias:
                            typeVal == "document_book" ? soServiceAlias : "",
                    },
                    success: function (data) {
                        response(
                            $.map(data, function (item) {
...
function initializeAutocompleteTED(
    selector,
    idSelector,
    nameSelector,
    type,
    ...
    fetch(checkBomJobUrl + "?item_id=" + itemId).then((response) => {
        return response.json().then((data) => {
            if (!data?.data?.is_bom) {
                ...
                $input
                    .closest("tr")
                    .find("input[name*='inventoty_uom_id']")
                    .val();
...
$(document).on("click", "#addNewItemBtn", (e) => {
    ...
    let actionUrl =
        ...
        rowsLength +
        "&component_item=" +
        encodeURIComponent(JSON.stringify(lastTrObj));
    fetch(actionUrl).then((response) => {
        return response.json().then((data) => {
            if (data.status == 200) {
                if (rowsLength) {
...
$(document).on(
    "change",
    ".po-order-detail > tbody .pi_item_checkbox",
    function (e) {
        const $checkbox = $(this);
        ...
        if ($checkbox.is(":checked")) {
            if (!selectedPwoId) {
                ...
                $(".po-order-detail > tbody .pi_item_checkbox").each(
                    function () {
            ...
            const anyChecked =
                $(".po-order-detail > tbody .pi_item_checkbox:checked").length >
                0;
            if (!anyChecked) {
                selectedPwoId = null;
            }
...
$(document).on(
    "change",
    ".po-order-detail > thead .form-check-input",
    function () {
        if ($(this).is(":checked")) {
            const firstCheckbox = $(
                ".po-order-detail > tbody .pi_item_checkbox"
            ...
            $(".po-order-detail > tbody .pi_item_checkbox").prop(
                "checked",
                false
            );
            selectedPwoId = null;
...
function getFirstNonEmptyPwoId() {
    const inputs = document.querySelectorAll(
        'input[name^="components"][name$="[pwo_id]"]'
    );
    ...
    fetch(actionUrl).then((response) => {
        return response.json().then((data) => {
            if (data.status == 200) {
                ...
                let existingIdsUpdate = JSON.parse(
                    localStorage.getItem("selectedPiIds")
                );
                if (
                    $("#itemTable .mrntableselectexcel").find("tr[id*='row_']")
                        .length
                ) {
                    $(
                        "#itemTable .mrntableselectexcel tr[id*='row_']:last"
                    ).after(data.data.pos);
                } else {
...
$(document).on("click", "#deleteBtn", (e) => {
    ...
    if (itemIds.length) {
        itemIds.forEach(function (item, index) {
            let piItemHiddenId = $(`#row_${item}`)
                .find("input[name*='[pwo_so_mapping_id]']")
                .val();
            if (piItemHiddenId) {
                ...
                if (selectedPiIds) {
                    selectedPiIds = JSON.parse(selectedPiIds);
                    let updatedIds = selectedPiIds.filter(
    ...
    $(document).on("click", "#deleteConfirm", (e) => {
        ...
        if (ids.length) {
            ids.forEach((id, index) => {
                let piItemHiddenId = $(`.form-check-input[data-id='${id}']`)
                    .closest("tr")
                    .find("input[name*='[pwo_so_mapping_id]']")
                    .val();
                if (piItemHiddenId) {
