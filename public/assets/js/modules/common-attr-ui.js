...
function setAttributesUIHelper(paramIndex = null, selectorPrifix = "") {
    ...
    const container =
        document.querySelector(selectorPrifix) ||
        document.querySelector("#itemTable tbody") ||
        document.querySelector(".grnItemsTable tbody");
    ...
    attributesArray.forEach((attrArr) => {
        ...
        let currentStringLength = short
            ? Number(attrArr.short_name.length)
            : Number(attrArr.group_name.length);
        ...
        if (attrTotalChar + Number(currentStringLength) <= 15) {
            attributeUI += `
            <span class="badge rounded-pill badge-light-primary"><strong>${
                short ? attrArr.short_name : attrArr.group_name
            }</strong>: ${
            ...
            if (remainingLength >= 3) {
                attributeUI += `<span class="badge rounded-pill badge-light-primary"><strong>${attrArr.group_name.substring(
                    0,
                    remainingLength - 1
                )}..</strong></span>`;
            } else {
    ...
    let attributeSection = container.querySelector(
        `[id="itemAttribute_${currentItemIndex}"]`
    );
    if (attributeSection) {
...
function setSelectedAttribute(rowCount) {
    ...
    if (attributesArray.length) {
        ...
        currentTr
            .find(`td[id="itemAttribute_${rowCount}"]`)
            .attr("attribute-array", JSON.stringify(attributesArray));
        setAttributesUIHelper(rowCount, ".grnItemsTable tbody");
    }
}
