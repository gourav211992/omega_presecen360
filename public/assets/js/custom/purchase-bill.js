// /*=========================================================================================
//     File Name: purchase-bill.js
//     Description: Purchase Bill report page content with filter
//     ----------------------------------------------------------------------------------------
...
function updateTable(ge_reports = [], columnVisibility = []) {
    ...
    const tableHeaders = [
        `<th>S No</th>`, // Index
        `<th>Series</th>`, // Series
        `<th>Purchase Bill No</th>`, // Purchase Bill No
        `<th>Purchase Bill Date</th>`, // Purchase Bill Date
        `<th>MRN No</th>`, // MRN No
        `<th>SO No</th>`, // SO No
        `<th>Vendor</th>`, // Vendor
    ...
    ge_reports.forEach((report) => {
        report.items.forEach((pb_item) => {
            const tr = document.createElement("tr");
            // Create table data cells based on column visibility

            total_item_value = ((pb_item?.rate ?? 0.00) * (pb_item?.accepted_qty ?? 0.00)) - (pb_item?.discount_amount ?? 0.00);
            let mrn_no = report?.mrn?.book_code ? report.mrn.document_number : '';
            let ge_no = report?.gate_entry_no ?? '';
            let so_no = pb_item?.so?.book_code ? pb_item.so.document_number : '';
            ...
            const cells = [
                ...
                `<td class="fw-bolder text-dark">${report.document_number}</td>`, // Purchase Bill No
                `<td>${formatDate(report.document_date)}</td>`, // Purchase Bill Date
                ...
                `<td>${pb_item?.item?.category?.name ?? ""}</td>`, // category
                `<td>${pb_item?.item?.sub_category?.name ?? ""}</td>`, // Sub category
                `<td>${pb_item?.item?.type ?? ""}</td>`, // item type
                `<td>${"sub type" ?? ""}</td>`, // sub type
                `<td>${pb_item?.item?.item_name ?? ""}</td>`, // Item Name
                `<td>${pb_item?.item?.item_code ?? ""}</td>`, // Item Code
                `<td>${Number(pb_item?.order_qty ?? 0).toFixed(2)}</td>`, // Order Quantity
                `<td>${Number(pb_item?.rate ?? 0).toFixed(2)}</td>`, // Rate
                `<td>${Number(pb_item?.basic_value ?? 0).toFixed(2)}</td>`,// Basic Value
                `<td>${Number(pb_item?.discount_amount ?? 0).toFixed(2)}</td>`, // Item Discount
                `<td>${Number(pb_item?.header_discount_amount ?? 0).toFixed(2)}</td>`, // Item Header Discount
                ...
                `<td class="no-wrap">
                    ...
                    <a href="/purchase-bills/edit/${report?.id}">
