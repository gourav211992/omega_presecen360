// /*=========================================================================================
//     File Name: purchase-return.js
//     Description: Purchase Return report page content with filter
//     ----------------------------------------------------------------------------------------
...
function updateTable(pr_reports = [], columnVisibility = []) {
    ...
    const tableHeaders = [
        `<th>S No</th>`, // Index
        `<th>Series</th>`, // Series
        `<th>Purchase Retrun No</th>`, // Purchase Retrun No
        `<th>Purchase Retrun Date</th>`, // Purchase Retrun Date
        `<th>MRN No</th>`, // MRN No
        `<th>SO No</th>`, // SO No
        `<th>Quantity Return Type</th>`, // Quantity Return Type
        `<th>Vendor</th>`, // Vendor
    ...
    pr_reports.forEach((report) => {
        report.items.forEach((pr_item) => {
            const tr = document.createElement("tr");
            // Create table data cells based on column visibility

            total_item_value = ((pr_item?.rate ?? 0.00) * (pr_item?.accepted_qty ?? 0.00)) - (pr_item?.discount_amount ?? 0.00);
            let mrn_no = report?.mrn?.book_code ? report.mrn.document_number : '';
            let ge_no = report?.gate_entry_no ?? '';
            let so_no = pr_item?.so?.book_code ? pr_item.so.document_number : '';
            ...
            const cells = [
                ...
                `<td class="fw-bolder text-dark">${report.document_number}</td>`, // Purchase Return No
                `<td>${formatDate(report.document_date)}</td>`, // Purchase Return Date
                `<td class="fw-bolder text-dark">${mrn_no ?? ""}</td>`, // MRN No
                `<td class="fw-bolder text-dark">${so_no ?? ""}</td>`, // SO No
                `<td>${report.qty_return_type.charAt(0).toUpperCase() + report.qty_return_type.slice(1).toLowerCase() ?? ""}</td>`, // Quantity Return Type
                `<td>${report.vendor?.display_name ?? ""}</td>`, // Vendor
                `<td>${"vendor rating" ?? ""}</td>`, // Vendor rating
                `<td>${pr_item?.item?.category?.name ?? ""}</td>`, // category
                `<td>${pr_item?.item?.sub_category?.name ?? ""}</td>`, // Sub category
                `<td>${pr_item?.item?.type ?? ""}</td>`, // item type
                `<td>${"sub type" ?? ""}</td>`, // sub type
                `<td>${pr_item?.item?.item_name ?? ""}</td>`, // Item Name
                `<td>${pr_item?.item?.item_code ?? ""}</td>`, // Item Code
                `<td>${Number(pr_item?.order_qty ?? 0).toFixed(2)}</td>`, // Order Quantity
                `<td>${Number(pr_item?.rate ?? 0).toFixed(2)}</td>`, // Rate
                `<td>${Number(pr_item?.basic_value ?? 0).toFixed(2)}</td>`,// Basic Value
                `<td>${Number(pr_item?.discount_amount ?? 0).toFixed(2)}</td>`, // Item Discount
                `<td>${Number(pr_item?.header_discount_amount ?? 0).toFixed(2)}</td>`, // Item Header Discount
                ...
                `<td class="no-wrap">
                    ...
                    <a href="/purchase-return/edit/${report?.id}">
