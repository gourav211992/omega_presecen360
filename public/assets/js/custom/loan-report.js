...
let totalRecoveryInterest=0;
    ...
    if(totalRecoveryPrincipal==0){
        ...
        let totalSettled =0;
        console.log(report.home_loan.settlement);
        if (report.home_loan.settlement) {
            totalSettled = Object.entries(report.home_loan.settlement) // Convert object to array
                .filter(item =>
                    item.document_status === 'Approved' ||
                    item.document_status === 'approval_not_required' ||
                    item.document_status === 'posted'
                ) // Filter based on approvalStatus
