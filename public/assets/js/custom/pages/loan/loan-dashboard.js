...
function filterLoanAnalytics(
    ...
    if (donutChartElroletask) {
        fetch(url)
            ...
            .then((data) => {
                // update data
                homeLoansElement.textContent = data.home_loans.toLocaleString();
                vehicleLoansElement.textContent =
                    data.vehicle_loans.toLocaleString();
                termLoansElement.textContent = data.term_loans.toLocaleString();
...
function filterLoanSummery(
    ...
    fetch(url)
        ...
        .then((data) => {
            ...
            let maxY = Math.max(...fetchedData);
            let desiredSteps = 7;

            let stepSize = Math.ceil(maxY / desiredSteps);

            // Adjust maxY to be a multiple of the new stepSize
            maxY = Math.ceil(maxY / stepSize) * stepSize; // Adjust maxY to a multiple of stepSize
