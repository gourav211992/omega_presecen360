...
function updateTable(data = [], columnVisibility = []) {
    ...
    const tableHeaders = [
        ...
        `<th>Name</th>`, // Name
        `<th>Approved Amount</th>`, // Approved Amount
        `<th>Disbursed Amount</th>`, // Disbursed Amount
        `<th>Disbursement Date</th>`, // Disbursement Date
        `<th>Pending Disbursement</th>`, // Pending Disbursement
    ];
    ...
    data.loan_reports.forEach((report, index) => {
        ...
        const cells = [
            ...
            `<td>${report.home_loan.loan_amount ?? "N/A"}</div></td>`,
            `<td>${report.totalDisAmount ?? "N/A"}</td>`,
            `<td>${formatDate(report.created_at) ?? "N/A"}</td>`,
            `<td>${report.pendingDisAmount ?? "N/A"}</td>`,
        ];
