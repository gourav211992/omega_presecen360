...
<style>
    ...
    <!-- BEGIN: Content-->
    <form method="POST" data-completionFunction = "disableHeader" class="ajax-input-form sales_module_form material_issue" action = "{{route('pq.store')}}" data-redirect="{{ $redirect_url }}" id = "sale_invoice_form" enctype='multipart/form-data'>
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="card">
                                <div class="card-body vendornewsection-form" id ="main_so_form">  
                                    <div class="row">
                                        ...
                                        <div class="col-md-8"> 
                                            ...
                                            @if (isset($order))
                                                <input type = "hidden" value = "{{$order -> id}}" name = "pq_header_id"></input>
                                            @endif
                                                ...
                                                <div class="col-md-5">  
                                                    <select class="form-select disable_on_edit except_draft" name="store_id" id="store_id_input">
                                                        @if(isset($order) && $order->store_id && $order->document_status != App\Helpers\ConstantHelper::DRAFT)
                                                            <option display-address = "{{$order->store -> address ?-> display_address}}" value="{{ $order->store_id }}" selected> {{ $order->store->store_name }}</option>
                                                        @else
                                ...
                                @include('pq.vendor_section', ['order' => $order ?? null, 'vendors' => $vendors ?? []])
                            </div>
...
initializeAutocompleteSearch('filter_item_name_code','filter_item_name_code_id','inventory_items','item_name');
            ...
            fetch(apiUrl, {
                ...
                if (mainDropdownElement.id == "vendor_id_input") {
                    if (data?.data?.currency_exchange?.status == false || data?.data?.error_message) {
                        ...
                        document.getElementById('current_billing_country_id').value = "";
                        document.getElementById('current_billing_state_id').value = "";
        ...
        $(document).on('click','#billAddressEditBtn',(e) => {
            ...
            fetch(apiUrl, {
                ...
                if (data) {
                    ...
                    $("#country_id_input_hidden").val(data.address.country_id);
                    $("#current_billing_state_id").val(data.address.state_id);
                    $("#state_id_input_hidden").val(data.address.state_id);
    ...
    function getOrders(type = "rfq")
    {
        ...
        const apiUrl = "{{ route('pq.get.items') }}";
    ...
    function processOrder()
    {
        ...
        const docType = $("#service_id_input").val();
        const apiUrl = "{{route('pq.process.items')}}";
        // const apiUrl = "{{('pq.process.items')}}";
        let docId = [];
        let soItemsId = [];
        let qties = [];
        let documentDetails = [];
                    ...
                    currentOrders.forEach((currentOrder) => {
                        currentOrder.items.forEach((item) => {
                            ...
                            <tr id="item_row_${itemRowIndex}" class="item_header_rows" onclick="onItemClick('${itemRowIndex}');">
                                ...
                                <input type="hidden" id="${openPullType}_id" name="${openPullType}_id" value="${JSON.stringify(item.header_ids[0])}">
                                ...
                                <td>
                                    <input type="text" id="item_rate_${itemRowIndex}" name="item_rate[${itemRowIndex}]" oninput="changeItemRate(this, ${itemRowIndex});" class="form-control mw-100 text-end" onblur="setFormattedNumericValue(this);" />
                                </td>
                                <td>
                                    <input type="text" id="item_value_${itemRowIndex}" disabled class="form-control mw-100 text-end item_values_input" />
                                </td>
                                <input type="hidden" id="header_discount_${itemRowIndex}" value="0" />
                                <input type="hidden" id="header_expense_${itemRowIndex}" />
                                <td>
                                    <div class="position-relative d-flex align-items-center">
                                        <input type="text" id="item_discount_${itemRowIndex}" disabled class="form-control mw-100 text-end item_discounts_input" style="width: 70px" />
                                        <div class="ms-50">
                                            <button type="button" onclick="onDiscountClick('item_value_${itemRowIndex}', ${itemRowIndex})" class="btn p-25 btn-sm btn-outline-secondary" style="font-size: 10px">Add</button>
                                        </div>
                                    </div>
                                </td>
                                <input type="hidden" id="item_tax_${itemRowIndex}" class="form-control mw-100 text-end item_taxes_input" style="width: 70px" />
                                <td><input type="text" id = "value_after_discount_${itemRowIndex}"  disabled class="form-control mw-100 text-end item_val_after_discounts_input" /></td>
                                <input type="hidden" id="value_after_header_discount_${itemRowIndex}" class="item_val_after_header_discounts_input" />
                                <input type="hidden" id="item_total_${itemRowIndex}" class="form-control mw-100 text-end item_totals_input" />
                                <td>
                                    <input type="text" id="item_remarks_${itemRowIndex}" name="item_remarks[]" class="form-control mw-100" placeholder="Enter Remarks">
                                </td>
                            </tr>`;
