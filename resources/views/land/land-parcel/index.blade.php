...
@section('content')
            ...
            <div class="content-header row">
                ...
                <div class="content-header-right text-sm-end col-md-6 mb-50 mb-sm-0">
                    <div class="form-group breadcrumb-right">
                        ...
                        <a href="{{ url('land-parcel/add') }}" class="btn btn-dark btn-sm mb-50 mb-sm-0"><i
                                        ...
                                        <table class="datatables-basic table myrequesttablecbox loanapplicationlist">
                                            ...
                                            <tbody>
                                                ...
                                                @foreach ($lands as $key => $land)
                                                    @php
                                                        $mainBadgeClass = match ($land->document_status) {
                                                            'approved' => 'success',
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>{{ $land->name }}</td>
                                                        <td>{{ $land->handoverdate }}</td>
                                                        <td class="fw-bolder text-dark">{{ $land->surveyno }}</td>
                                                        <td>{{ $land->plot_area }}({{ $land->area_unit }})</td>
                                                        <td>{{ $land->address }}</td>
                                                        <td>{{ $land->pincode }}</td>
                                                        <td>{{ $land->land_valuation }}</td>
                                                        <td>
                                                            <span
                                                                class="badge rounded-pill badge-light-{{ $mainBadgeClass }} badgeborder-radius">{{ $land->document_status == 'approval_not_required' ? 'Approved' : Helper::formatStatus($land->document_status) }}</span>
                                                        </td>
                                                        <td class="tableactionnew">
                                                            <div class="dropdown">
                                                                ...
                                                                @php
                                                                    $creatorType = explode('\\', $land->type);
                                                                    $buttons = Helper::actionButtonDisplay(
                                                                        $land->book_id,
                                                                        $land->document_status,
                                                                        $land->id,
                                                                        0,
                                                                        $land->approval_level,
                                                                        $land->created_by,
                                                                        strtolower(end($creatorType)),
                                                                    );
                                                                @endphp
                                                                <div class="dropdown-menu dropdown-menu-end">
                                                                    @if ($buttons['draft'])
                                                                        @if ($land->document_status == 'draft')
                                                                            <a class="dropdown-item"
                                                                                href="{{ url('/land-parcel/edit/' . $land->id) }}">
                                                                    ...
                                                                    <a class="dropdown-item"
                                                                        href="{{ url('/land-parcel/view/' . $land->id) }}">
    ...
    <div class="modal modal-slide-in fade filterpopuplabel" id="filter">
        <div class="modal-dialog sidebar-sm">
            <form class="add-new-record modal-content pt-0" method="GET" action="{{ route('land-parcel.filter') }}">>
                    ...
                    <div class="mb-1">
                        ...
                        <select class="form-select select2" name="land_no">
                            ...
                            @foreach ($land_no as $option)
                                <option value="{{ $option }}" {{ $option == old('land_no') ? 'selected' : '' }}>
                                    {{ $option }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="mb-1">
                        ...
                        <select class="form-select select2" name="pincode">
                            ...
                            @foreach ($pincode as $option)
                                <option value="{{ $option }}" {{ $option == old('pincode') ? 'selected' : '' }}>
                                    {{ $option }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="mb-1">
                        ...
                        <select class="form-select" name="selectedStatus">
                            ...
                            @foreach ($selectedStatus as $option)
                                <option value="{{ $option }}" {{ $option == old('pincode') ? 'selected' : '' }}>
                                    {{ $option == 'approval_not_required' ? 'Approved' : Helper::formatStatus($option) }}
                                </option>
                            @endforeach
                        </select>
                    </div>
