@foreach($items as $key => $item)
    ...
    <tr id="row_{{$rowCount}}" data-index="{{$rowCount}}" @if($rowCount < 2 ) class="trselected" @endif>
        ...
        <td>
            <input type="text" class="form-control mw-100 accepted_qty" name="components[{{$rowCount}}][accepted_qty]" value="{{($item->order_qty - $item->grn_qty)}}" />
        </td>qty
        <td>
            <input type="text" name="components[{{$rowCount}}][rate]" value="{{$item->rate}}" readonly class="form-control mw-100 text-end rate" />
        </td>
        <td>
            <input type="text" name="components[{{$rowCount}}][basic_value]" value="{{($item->order_qty - $item->grn_qty)*$item->rate}}"  class="form-control text-end mw-100 basic_value" readonly />
        </td>
        <td>
            <div class="position-relative d-flex align-items-center">
                ...
                @endforeach
                <input type="number" readonly name="components[{{$rowCount}}][discount_amount]" class="form-control mw-100 text-end" style="width: 70px" value="{{$totalValue}}" />
