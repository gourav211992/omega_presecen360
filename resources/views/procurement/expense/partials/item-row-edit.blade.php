@foreach($mrn->items as $key => $item)
   ...
   <tr id="row_{{$rowCount}}" data-index="{{$rowCount}}" @if($rowCount < 2 ) class="trselected" @endif>
      ...
      <td>
         <input type="number" class="form-control mw-100 accepted_qty checkNegativeVal" name="components[{{$rowCount}}][accepted_qty]" value="{{$item->accepted_qty}}" step="any" @readonly(true) />
      </td>
      <td>
