...
@section('content')
    <form class="ajax-input-form" data-module="exp" method="POST" action="{{ route('exp-allocation.store') }}"
                                                    ...
                                                    <div class="col-md-3">
                                                        ...
                                                        <div class="mb-1 customer-part" style="display: none;">
                                                            ...
                                                            <input type="hidden" id="customer_code"
                                                                name="customer_code" />
                                                            <input type="hidden" id="cust_shipping_id"
                                                                name="cust_shipping_id" />
                                                            <input type="hidden" id="cust_billing_id"
                                                                name="cust_billing_id" />
                                                        </div>
                                                    </div>
                                                    <div class="col-md-3">
                                                        <div class="mb-1">
                                                            ...
                                                            <div class="bilnbody">
                                                                <div class="genertedvariables genertedvariablesnone">
                                                                    <label class="form-label w-100">
                                                                        Select Vendor Address <span
                                                                            class="text-danger">*</span>
                                                                        <a href="javascript:;"
                                                                            class="float-end font-small-2 editAddressBtn"
                                                                            id="editShippingAddressBtn"
                                                                            data-type="shipping">
                                                    ...
                                                    <div class="col-md-12">
                                                        <div class="table-responsive pomrnheadtffotsticky">
                                                            <table id="poItemsTable"
                                                                ...
                                                                <tbody class="poItemsTbody" id="poItemsTbody"></tbody>
                                                                <tfoot id="poItemsTfoot">
                                                                </tfoot>
                                                            </table>
                                                        </div>
                                                    </div>
        ...
        @include('procurement.expense-allocation.partials.summary-disc-modal')
        {{-- Add expenses modal --}}
        @include('procurement.expense-allocation.partials.summary-exp-modal')
        {{-- Add Outstanding PO modal --}}
        @include('procurement.expense-allocation.partials.outstanding-po-modal')
        {{-- Add Outstanding JO modal --}}
        @include('procurement.expense-allocation.partials.outstanding-jo-modal')
        {{-- Add Outstanding SO modal --}}
        <!-- @include('procurement.expense-allocation.partials.outstanding-so-modal') -->
        {{-- Edit Address --}}
    ...
    {{-- Taxes --}}
    @include('procurement.expense-allocation.partials.tax-detail-modal')
@endsection
@section('scripts')
    <script type="text/javascript">
        let actionUrlTax = '{{ route('exp-allocation.tax.calculation') }}';
    ...
    <script>
        ...
        function setServiceParameters(parameters) {
            ...
            if (reference_from_service.length) {
                ...
                setTimeout(() => {
                    location.href = '{{ route('exp-allocation.index') }}';
                }, 1500);
            }
        }
        ...
        function vendorOnChange(vendorId, type = null, typeId = null) {
            ...
            let actionUrl = "{{ route('exp-allocation.get.address') }}" +
                '?id=' + vendorId +
                '&store_id=' + store_id +
                '&type=' + type +
                '&typeId=' + typeId;
        ...
        $(document).on('click', '.editAddressBtn', (e) => {
            ...
            let actionUrl =
                `{{ route('exp-allocation.edit.address') }}?type=${addressType}&vendor_id=${vendorId}&address_id=${addressId}&onChange=${onChange}`;
            fetch(actionUrl)
        ...
        $(document).on('click', '.submitAddress', function(e) {
            ...
            var method = "POST";
            var url = '{{ route('exp-allocation.address.save') }}';
            fetch(url, {
                    method: method,
                    body: innerFormData,
                    headers: {
        ...
        function getPurchaseOrders() {
            const ajaxUrl = '{{ route('exp-allocation.get.po', ['type' => 'create']) }}';
            var columns = [];
            columns = [{
                    data: 'id',
                    visible: false,
        ...
        function openJobRequest() {
            initializeAutocompleteJoQt("jo_vendor_code_input_qt", "jo_vendor_id_qt_val", "vendor_list", "vendor_code",
                "company_name");
        ...
        function getJobOrders() {
            const ajaxUrl = '{{ route('exp-allocation.get.jo', ['type' => 'create']) }}';
            var columns = [];
            columns = [{
                    data: 'id',
                    visible: false,
        ...
        function asnProcess(asnData, moduleProcess) {
            ...
            const baseRoute = processType === 'jo' ?
                '{{ route('exp-allocation.process.jo-item') }}' :
                '{{ route('exp-allocation.process.po-item') }}';
