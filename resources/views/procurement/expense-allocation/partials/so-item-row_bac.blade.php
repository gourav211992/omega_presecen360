@foreach($soItems as $key => $item)
    ...
    <tr id="row_{{$rowCount}}" data-index="{{$rowCount}}" @if($rowCount < 2 ) class="trselected" @endif>
        ...
        <td>
            <input type="number" class="form-control mw-100 accepted_qty checkNegativeVal" name="components[{{$rowCount}}][accepted_qty]" value="{{($item->order_qty - $item->invoice_qty)}}" step="any"/>
        </td>
        <td>
        ...
        <td>
            <input type="number" name="components[{{$rowCount}}][basic_value]" value="{{($item->order_qty - $item->invoice_qty)*$item->rate}}"  class="form-control text-end mw-100 basic_value checkNegativeVal" readonly step="any"/>
        </td>
        <td>
            <div class="position-relative d-flex align-items-center">
                ...
                <input type="hidden" name="components[{{$rowCount}}][exp_amount_header]" value="{{$item->header_expense_amount}}" />
                <div class="ms-50">
