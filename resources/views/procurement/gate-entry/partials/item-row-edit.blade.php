@foreach($mrn->items as $key => $item)
   ...
   <tr id="row_{{$rowCount}}" data-index="{{$rowCount}}" @if($rowCount < 2 ) class="trselected" @endif>
      ...
      <td>
         <input type="number" name="components[{{$rowCount}}][rate]" value="{{number_format(($item->rate), 2, '.', '')}}" class="form-control mw-100 text-end rate checkNegativeVal" step="any" readonly />
      </td>
      <td>
