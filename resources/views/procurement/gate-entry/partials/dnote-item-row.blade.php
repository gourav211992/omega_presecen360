@foreach($soItems as $key => $item)
    ...
    <tr data-group-item="{{json_encode($item)}}" id="row_{{$rowCount}}" data-index="{{$rowCount}}" @if($rowCount < 2 ) class="trselected" @endif>
        ...
        <input type="hidden" name="components[{{$rowCount}}][dnote_item_hidden_ids]" value="{{$item->id}}">
        <input type="hidden" name="components[{{$rowCount}}][dnote_hidden_ids]" value="{{$item->sale_invoice_id }}">
