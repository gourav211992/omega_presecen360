                                    ...
                                    <div class="tab-content" id="productTabsContent">
                                        ...
                                        @if (isset($isRawMaterial) && $isRawMaterial)
                                            <div class="tab-pane fade" id="raw-materials" role="tabpanel" aria-labelledby="raw-materials-tab">
                                                <div class="table-responsive pomrnheadtffotsticky">
                                                    <table id="itemTable2" class="table myrequesttablecbox table-striped po-order-detail custnewpo-detail border newdesignerptable newdesignpomrnpad" data-json-key="component_json" data-row-selector="tr[id^='row_']">
                                                        <thead>
                                                            <tr>
                                                                {{-- <th class="customernewsection-form">
                                                 <div class="form-check form-check-primary custom-checkbox">
...
@section('scripts')
    ...
    <script>
        ...
        @if ($buttons['amend'] && intval(request('amendment') ?? 0))
            $('#job_order_type').prop('disabled', true);
        @else
        ...
        $(document).on('click', '#shortCloseBtn', (e) => {
            ...
            $('#itemTable > tbody .form-check-input').each(function() {
                if ($(this).is(":checked")) {
                    if (Number($(this).closest('tr').find('[name*="jo_product_id"]').val())) {
                        itemIds.push(Number($(this).closest('tr').find('[name*="jo_product_id"]').val()));
                    }
                }
            });
            if (itemIds.length) {
