@foreach ($poItems as $key => $item)
    @php
        ...
        $joQty = $item->avail_order_qty ?? 0.0;
    ...
    <tr data-group-item="{{ json_encode($item) }}" id="row_{{ $rowCount }}" data-index="{{ $rowCount }}"
        ...
        <td>
            <input type="number" class="form-control mw-100 accepted_qty text-end checkNegativeVal"
                name="components[{{ $rowCount }}][accepted_qty]"
                value="{{ $item?->item?->is_inspection == 0 ? $availableQty : 0.0 }}" step="any"
                {{ $acceptedReadOnly }} />
        </td>
        <td>
        ...
        <td>
            <input type="number" name="components[{{ $rowCount }}][basic_value]"
                value="{{ ($item->order_qty - $item->grn_qty) * $item->rate }}"
