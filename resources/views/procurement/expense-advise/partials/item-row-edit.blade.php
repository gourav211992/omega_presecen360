@foreach ($mrn->items as $key => $item)
    ...
    <tr id="row_{{ $rowCount }}" data-index="{{ $rowCount }}"
        ...
        <input type="hidden" name="components[{{ $rowCount }}][purchase_order_id]"
            value="{{ $item->expenseHeader->purchase_order_id }}">
        <input type="hidden" name="components[{{ $rowCount }}][po_detail_id]"
            value="{{ @$item->purchase_order_item_id }}">
        <input type="hidden" name="components[{{ $rowCount }}][job_order_id]"
            value="{{ $item->jo_id }}">
        ...
        @if ($mrn?->reference_type != 'jo')
            <td class="poprod-decpt attributeBtn" id="itemAttribute_{{ $rowCount }}"
                data-count="{{ $rowCount }}" attribute-array="{{ $item->item_attributes_array() }}">
            </td>
        @else
            <td></td>
        @endif
        ...
        <td>
            <input type="number" class="form-control mw-100 text-end accepted_qty"
                name="components[{{ $rowCount }}][accepted_qty]" value="{{ $item->accepted_qty }}" readonly
                step="any" />
        </td>
        <td>
        ...
        <td>
            <input type="number" name="components[{{ $rowCount }}][basic_value]"
                value="{{ $item->accepted_qty * $item->rate }}" class="form-control text-end mw-100 basic_value"
                step="any" readonly />
            <input type="hidden" name="components[{{$rowCount}}][po_b_value]"
                value="{{($item->accepted_qty*$item->po_rate)}}"  class="form-control text-end mw-100 basic_value"
                readonly step="any"/>
        </td>
        <td>
        ...
        <td>
            ...
            <input type="hidden" id="item_total_po_cost_{{$rowCount}}" name="components[{{$rowCount}}][po_total_cost]"
                value="{{($item->accepted_qty*$item->po_rate) - $item->discount_amount}}" readonly
