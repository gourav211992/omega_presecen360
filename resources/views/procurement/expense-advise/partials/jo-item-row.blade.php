@foreach ($joItems as $key => $item)
    @php
        ...
        $poQty = $item->avail_order_qty ?? 0.0;
    ...
    <tr data-group-item="{{ json_encode($item) }}" id="row_{{ $rowCount }}" data-index="{{ $rowCount }}"
        ...
        <td>
            <input type="text" name="component_item_name[{{ $rowCount }}]" placeholder="Select"
                class="form-control mw-100 ledgerselecct comp_item_code" value="{{ $item?->serviceItem?->item_code }}"
                readonly />
            ...
            <input type="hidden" name="components[{{ $rowCount }}][item_code]"
                value="{{ @$item?->serviceItem?->item_code }}" />
            ...
            <input type="hidden" name="components[{{ $rowCount }}][hsn_code]"
                value="{{ $item?->serviceItem?->hsn?->code }}" />
            <input type="hidden" name="components[{{ $rowCount }}][so_id]" value="{{ $item?->so_id }}">
        <td>
            <input type="text" name="components[{{ $rowCount }}][item_name]"
                value="{{ $item?->serviceItem?->item_name }}" class="form-control mw-100 mb-25" readonly />
        </td>

        {{-- @if ($item?->serviceItem?->itemAttributes)
                @php
                    $selectedAttr = @$item?->serviceItem?->itemAttributes ? @$item?->serviceItem?->itemAttributes()->whereNotNull('attribute_value')->pluck('attribute_value')->all() : [];
                @endphp
                @foreach (@$item?->serviceItem?->itemAttributes as $attributeHidden)
                ...
                @foreach (@$item?->serviceItem?->itemAttributes as $itemAttribute)
                    @if (count($selectedAttr))
                        @foreach ($itemAttribute->itemAttributes() as $value)
        ...
        <td>
            <input type="number" class="form-control mw-100 accepted_qty text-end checkNegativeVal"
                name="components[{{ $rowCount }}][accepted_qty]" value="{{ $availableQty }}" step="any"
                readOnly />
        </td>
        <td>
        ...
        <input type="hidden" name="components[{{ $rowCount }}][jo_service_item_id]"
            value="{{ $item->service_item_id }}">
        <input type="hidden" name="components[{{ $rowCount }}][jo_item_hidden_ids]"
            value="{{ $item->id }}">
        <input type="hidden" name="components[{{ $rowCount }}][jo_hidden_ids]" value="{{ $item->jo_id }}">
        <input type="hidden" name="components[{{ $rowCount }}][expense_advise_qty]"
