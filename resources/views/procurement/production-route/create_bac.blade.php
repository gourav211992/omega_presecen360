...
<script>
    $(document).ready(function () {
        ...
        function addNewLevel(afterLevel = null) {
            ...
            const newLevel = `
                ...
                <tr class="child-row" data-index="${levelId}" data-detail-id="1">
                    ...
                    <td>
                        <input type="text" name="levels[${levelId}][details][1][semi_finished_item_id]" placeholder="Select" class="form-control mw-100 mb-25 ledgerselecct pr_items " />
                        <input type="hidden" id="semi_finished_item_id_${levelId}_1" name="levels[${levelId}][details][1][semi_finished_item_id]" />
                        <input type="hidden" id="semi_finished_item_code_${levelId}_1" name="levels[${levelId}][details][1][item_code]" />
                    </td>
                    <td class="attributes-container">
                    </td>
                    <td>
                        <a class="text-primary btn-add-child" data-index="${levelId}">
                            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-plus-square">
                                <rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect>
                                <line x1="12" y1="8" x2="12" y2="16"></line>
                                <line x1="8" y1="12" x2="16" y2="12"></line>
                        ...
                        <a class="delete-child text-danger">
                            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-trash-2">
                                <polyline points="3 6 5 6 21 6"></polyline>
                                <path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path>
                                <line x1="10" y1="11" x2="10" y2="17"></line>
        ...
        // Also make the parent stations readonly for already selected stations
        $('.parent').each(function () {
            let $parentSelect = $(this);
            $parentSelect.find('option').each(function () {
                let $option = $(this);
                if ($option.val() !== "" && selectedStations.has($option.val())) {
                    //   $option.addClass('readonly').prop('disabled', false);
        ...
        function updateStationDropdown() {
            ...
            $('.station').each(function () {
                let $select = $(this);
                let levelId = $(this).closest('tr').data('index');
        ...
        $(document).on("change", ".parent", function () {
            ...
            const levelId = $(this).closest('tr').data('index');
            ...
            // Re-enable the previously disabled stations
            updateStationDropdown();
        });
        ...
        $('#levelTable').on('click', '.btn-add-child', function () {
            ...
            const newChild = `
                <tr class="child-row" data-index="${levelId}" data-detail-id="${detailCount}">
                    ...
                    <td>
                        <input type="text" name="levels[${levelId}][details][${detailCount}][semi_finished_item_id]" placeholder="Select" class="form-control mw-100 mb-25 ledgerselecct pr_items " />
                        <input type="hidden" id="semi_finished_item_id_${levelId}_${detailCount}" name="levels[${levelId}][details][${detailCount}][semi_finished_item_id]" />
                        <input type="hidden" id="semi_finished_item_code_${levelId}_${detailCount}" name="levels[${levelId}][details][${detailCount}][item_code]" />
                    </td>
                    <td class="attributes-container">
                    </td>
                    <td>
        ...
        $('#levelTable').on('click', '.deleteLevel', function () {
            const levelId = $(this).data('index'); // Use 'data-index'
