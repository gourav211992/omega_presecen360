@foreach ($itemAttributes as $index => $attribute)
    <tr>
        ...
        <td>
            <select {{ isset($isSo) && $isSo ? 'disabled' : '' }} class="form-select select2" name="comp_attribute[{{ $rowCount }}][item_name]" data-attr-name="{{ $attribute->attributeGroup->name }}" data-attr-group-id="{{ $attribute->attributeGroup->id }}">
