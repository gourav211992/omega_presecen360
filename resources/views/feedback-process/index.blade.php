            ...
            <div class="content-header">
                <div class="row">
                            ...
                            <div class="col-12">
                                ...
                                <div class="breadcrumb-wrapper">
                                    <ol class="breadcrumb">
                                        ...
                                        <li class="breadcrumb-item active">Feedback Process List</li>
                                    </ol>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="content-header-right text-end col-md-6 col-6 mb-2 mb-sm-0">
                        ...
                        <a class="btn btn-primary btn-sm" href="{{ route('feedback-process.create') }}"><i data-feather="plus-circle"></i> Add New</a>
                    </div>
                </div>
            </div>
        ...
        $(document).ready(function() {
            ...
            if (dt_basic_table.length) {
                var dt_basic = dt_basic_table.DataTable({
                    processing: true,
                    serverSide: true,
                    ajax: "{{ route('feedback-process.index') }}",  // Ajax route to fetch data
                    columns: [
                        ...
                        { data: 'user_name', name: 'user.name' },  // User Name (from user relationship)
                        { data: 'interaction_type_name', name: 'interactionType.name' },  // Interaction Type (from interactionType relationship)
                        { data: 'comment', name: 'comment' },  // Comment
                        { data: 'action', name: 'action' },  // Action Taken
                        { data: 'action_buttons', orderable: false, searchable: false }  // Action buttons
                    ],
            ...
            $(".apply-filter").on("click", function () {
                ...
                var interactionType = $("#filter-interaction-type").val();
                dt_basic.ajax.url("{{ route('feedback-process.index') }}?date_range=" + dateRange + "&customer_name=" + customerName + "&interaction_type=" + interactionType).load();
                $(".modal").modal("hide");
            });
        });
    </script>
