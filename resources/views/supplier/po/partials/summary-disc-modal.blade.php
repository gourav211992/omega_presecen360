            ...
            <div class="modal-body px-sm-2 mx-50 pb-2">
                ...
                <div class="table-responsive-md customernewsection-form">
                    <table id="summaryDiscountTable" class="mt-1 table myrequesttablecbox table-striped po-order-detail custnewpo-detail"> 
                        ...
                        <tbody>
                     @if(isset($po->headerDiscount) && $po->headerDiscount->count())
                     @foreach($po->headerDiscount as $hd_key => $headerDiscount)
                     <tr class="display_summary_discount_row">
                        ...
                        <td class="text-end">{{floatval($headerDiscount->ted_perc) ?? 0}}
                            <input type="hidden" value="{{floatval($headerDiscount->ted_perc) ?? ''}}" name="disc_summary[{{$hd_key+1}}][d_perc]" />
                        </td>
                        <td class="text-end">{{floatval($headerDiscount->ted_amount) ?? ''}}
                            <input type="hidden"  value="{{$headerDiscount->ted_amount ?? ''}}" name="disc_summary[{{$hd_key+1}}][d_amnt]" /></td>
                            <td>
                       ...
                       <tr id="disSummaryFooter">
                         ...
                         <td class="text-dark text-end"><strong id="total" amount="{{@$po->total_header_disc_amount}}">{{number_format(@$po->total_header_disc_amount,2)}}</strong></td>
                         <td></td>
                     </tr>
                 </tbody>
             </table>
