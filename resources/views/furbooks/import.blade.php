        ...
        <div class="content-header row">
            <div class="content-header-left col-md-5 mb-2">
                <div class="row breadcrumbs-top">
                    <div class="col-12">
                        <h2 class="content-header-title float-start mb-0">Upload Furbook Data</h2>
                        <div class="breadcrumb-wrapper">
                            <ol class="breadcrumb">
                                <li class="breadcrumb-item active">Furbook Import</li>
                            </ol>
                        </div>
                    </div>
                </div>
            ...
            <div class="content-header-right text-sm-end col-md-7 mb-50 mb-sm-0">
                <div class="form-group breadcrumb-right">
                    <a href="{{ route('furbooks.index') }}" class="btn btn-secondary btn-sm mb-50 mb-sm-0">
                        <i data-feather="arrow-left-circle"></i> Back
                    </a>
                    <a href="{{ asset('templates/Furbook_sample_template.xlsx') }}" class="btn btn-secondary btn-sm" id="download-template-btn" download>
        ...
        <div class="content-body">
            <section id="basic-datatable">
                <div class="row justify-content-center">
                    <div class="col-9">
                        <form class="importForm" method="POST" action="{{ route('furbooks.import.post') }}" enctype="multipart/form-data">
                            @csrf
...
<script>
            ...
            success: function (res) {
                ...
                $('.exportBtn').on('click', function() {
                    ...
                    if (activeTab === 'successful-records') {
                        window.location.href = `/furbooks/export-successful`; 
                    } else if (activeTab === 'failed-records') {
                        window.location.href = `/furbooks/export-failed`; 
                    }
                });
            },
    ...
    function populateTable(tableBodySelector, furbooks) {
        console.log(furbooks);
            const tableBody = $(tableBodySelector);
            tableBody.empty();

            if (furbooks.length > 0) {
                furbooks.forEach((furbook, index) => {
                    const row = `
                        <tr>
                            <td>${index + 1}</td>
                            <td class="fw-bolder text-dark">${furbook.furbooks_code || 'N/A'}</td>
                            <td>${furbook.credit_amount || 0}</td>
                            <td>${furbook.debit_amount || 0}</td>
                            <td>${furbook.amount || 0}</td>
                            <td class="${furbook.status === 'success' ? 'text-success' : 'text-danger'}">
                                ${furbook.status === 'success' ? 'Success' : (furbook.status === 'failed' ? 'Failed' : furbook.status)}
                            </td>
                             <td class="${furbook.remarks ? 'text-danger' : 'text-success'}">
                                ${furbook.remarks || 'Successfully uploaded'}
                            </td>
                        </tr>
                    `;
                ...
                const noDataRow = `<tr><td colspan="7" class="text-center">No records found</td></tr>`;
