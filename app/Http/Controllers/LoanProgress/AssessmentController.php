...
class AssessmentController extends Controller
{
    ...
    public function viewHomeLoan($id)
    {
       ...
       $book_type = Helper::getBookSeriesNew($firstService -> alias, $parentURL)->get();
        ...
        $loan_appraisal_id = $overview ? $overview->id : null;
        // if (!$overview) {
        //     $overview = (object) [
        //         'created_at' => 'N/A',
        //         'unit_name' => 'N/A',
        ...
        $credit_scoring = LoanAppraisalScoring::where('loan_id',$homeLoan->id)->first();
        return view('loanProgress.viewHomeLoan', compact('banks', 'ledgers','homeLoan', 'occupation', 'series', 'book_type', 'logsGroupedByStatus', 'view_detail', 'interest_rate', 'page', 'overview', 'document_listing', 'loan_disbursement', 'recovery_loan', 'buttons','logs','credit_scoring','loan_appraisal_id'));
    }
...
$buttons = Helper::actionButtonDisplayForLoan($vehicleLoan->series, $vehicleLoan->approvalStatus,$vehicleLoan->id,$vehicleLoan->loan_amount,$vehicleLoan->approval_level,$vehicleLoan->loanable_id,$vehicleLoan->loanable_type);        $logs = Helper::getLogs($id);
        ...
        $credit_scoring = LoanAppraisalScoring::where('loan_id',$vehicleLoan->id)->first();
    ...
    public function viewTermLoan($id)
    {
        ...
        $credit_scoring = LoanAppraisalScoring::where('loan_id',$termLoan->id)->first();
        return view('loanProgress.viewTermLoan', compact('banks', 'ledgers','termLoan','overview', 'series', 'book_type', 'logsGroupedByStatus', 'view_detail', 'interest_rate', 'page', 'overview', 'document_listing', 'loan_disbursement', 'recovery_loan', 'buttons','logs','credit_scoring','loan_appraisal_id'));
    }
