...
class CrDrReportController extends Controller
{
        ...
        foreach ($ledger_groups as $group) {
            ...
            if ($ledgers) {
				...
				$l_ledger = ItemDetail::whereIn('ledger_id', $ledgers)
					...
					->with(['ledger:id,name', 'voucher:id,document_date']) // Eager load related data
					->get()
					->groupBy('ledger_id')
					->map(function ($items) use ($ages0, $ages1, $ages2, $ages3, $ages4, $amount,$due_date) {
						...
						foreach ($items as $item) {
							...
							$totals->ledger_name = $item->ledger->name ?? '';
							// Preload group data to avoid N+1 queries
							static $groupCache = [];
							$groupId = $item->ledger_parent_id;
							if (!isset($groupCache[$groupId])) {
								$groupCache[$groupId] = Group::find($groupId);
							}
							$totals->ledger_parent_name = $groupCache[$groupId]->name ?? '';
							$totals->ledger_parent_id = $groupId;
							
							$d_date = ($due_date =="invoice" || empty($item->due_date)) 
								? ($item->voucher->document_date ?? null): $item->due_date;
                ...
                foreach ($l_ledger as $customer) {
                    ...
                    // Cache credit_days to avoid repeated database lookups
                    static $creditDaysCache = [];
                    $cacheKey = $model . '_' . $group . '_' . $ledger;
                    if (!isset($creditDaysCache[$cacheKey])) {
                        $creditDaysCache[$cacheKey] = $model::where('ledger_group_id', $group)
                            ->where('ledger_id', $ledger)
                            ->value('credit_days');
                    }
                    $credit_days = $creditDaysCache[$cacheKey];

                    // Optimize: Get all overdue values in one call instead of 7 separate calls
                    $overdueData = CrDrHelper::get_overdue_all_values($type, $ages_all, $doc_types, $cus_type, $voucher, $credit_days, $group, $ledger, $start, $end, 'overdue',$due_date);
                    $overdue = $overdueData['overdue'];
                    $ages0 = $overdueData['days_0_30'];
                    $ages1 = $overdueData['days_30_60'];
                    $ages2 = $overdueData['days_60_90'];
                    $ages3 = $overdueData['days_90_120'];
                    $ages4 = $overdueData['days_120_180'];
                    $ages5 = $overdueData['days_above_180'];
                    $total_outstanding = $overdueData['total_outstanding'];
    ...
    public static function get_bucket($diffDays)
    {
