...
namespace App\Http\Controllers\ErpTransaction;
...
class IndexController extends Controller
{
        ...
        return $this->handleTransactionView(
            ...
            userFilter: fn($query, $user) => $query->whereExists(function ($query) use ($user) {
                $query->select(DB::raw(1))
                    ...
                    ->where('erp_approval_workflows.user_id', $user->id)
    ...
    public function requests(Request $request)
    {
        return $this->handleTransactionView(
            ...
            userFilter: fn($query, $user) => $query->where('created_by', $user->id)
            // userFilter: fn($query, $user) => dd($user)
        );
    }
    ...
    private function handleTransactionView(Request $request, string $view, string $redirectRoute, array $documentStatuses, callable $userFilter = null, bool $excludeOwn = false)
    {
        ...
        if ($request->ajax()) {
            ...
            return $dataTable
                ...
                ->addColumn('party_name', fn($row) => $row->getPartyDisplayName() ?? ($row -> party_code ?? 'NA'))  
    ...
    private function renderStatusWithActions($row): string
    {
        ...
        $allRouteParams = array_fill_keys(['id','payment','voucher','receipt','ledger'], $row->document_id) + ['type' => $documentType];
