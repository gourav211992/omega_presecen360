...
class StoreService
{
    ...
    public function store(array $inputData, $expense, $user, $organization): array
    {
        ...
        try {
            ...
            foreach ($poLines as $idx => $row) {
                ...
                $poDetail->exchange_rate = $row['exchange_rate'] ?? null;
                ...
                //         if (isset($inputData['attr_group_id'][$itemAttribute->attribute_group_id])) {
                //             $attrName = $inputData['attr_group_id'][$itemAttribute->attribute_group_id]['attr_name'];
                //             $grnAttr = self::saveAttribute($expense, 'po', $poDetail, $itemAttribute, $attrName); // fixed: pass $grnDetail
            ...
            foreach ($grnLines as $idx => $row) {
                ...
                $grnDetail->exchange_rate = $row['exchange_rate'] ?? null;
                ...
                if (!empty($row['attr_group_id']) && is_array($row['attr_group_id'])) {
                    #Save component Attr
                    foreach ($grnDetail->item->itemAttributes as $itemAttribute) {
                        if (isset($row['attr_group_id'][$itemAttribute->attribute_group_id])) {
                            $attrName = $row['attr_group_id'][$itemAttribute->attribute_group_id]['attr_name'];
                            $grnAttr = self::saveAttribute($expense, 'grn', $grnDetail, $itemAttribute, $attrName); // fixed: pass $grnDetail
                            $createdGrnAttrIds[] = $grnAttr->id;
                        }
                    }
                }
    ...
    private static function saveAttribute($expense, $type, $detail, $itemAttribute, $attrName)
    {
        if ($type === 'po') {
        ...
        $attribute->item_attribute_id = $itemAttribute->id;
        $attribute->item_code = $detail->item_code;
        $attribute->attr_name = $itemAttribute->attribute_group_id;
        $attribute->attr_value = $attrName ?? null;
        $attribute->save();
        return $attribute;
    }
