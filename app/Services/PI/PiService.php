...
class PiService
{
    ...
    {
        try {
            $attrsJson = json_encode($attributes, JSON_UNESCAPED_UNICODE);
            $bomCheck = ItemHelper::checkItemBomExists($itemId, $attributes);

            $bom = Bom::find($bomCheck['bom_id']);
            if (!$bom) {
                return ['status' => 422, 'message' => "BOM not found for item {$itemId}"];
            }
            ...
            foreach ($bomDetails as $bomDetail) {
                ...
                $bomDetailQty = floatval($bomDetail->qty);
                $requiredQty = floatval($qty) * $bomDetailQty;
                if ($bufferPerc > 0) {
                    $requiredQty += ($requiredQty * $bufferPerc / 100);
                }
                ...
                $mappingData = [
                    ...
                    'vendor_id' => $bomDetail->vendor_id ?? null,
                    'item_code' => $bomDetail->item_code ?? null,
                    'order_qty' => $soItemOrderQty,
                    'bom_qty' => $bomDetailQty,
                    'qty' => $requiredQty,
                ...
                $data = PiSoMapping::updateOrCreate(
                    [
                        'so_id' => $soId,
                        'so_item_id' => $soItemId,
                        'item_id' => $bomDetail->item_id,
            ...
            return ['status' => 200, 'message' => 'Saved! (top-level BOM only)'];
        } catch (\Throwable $e) {
            return ['status' => 422, 'message' => $e->getMessage()];
        }
    }
