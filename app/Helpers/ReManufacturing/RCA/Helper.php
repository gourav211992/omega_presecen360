...
class Helper
{
   public static function generateRcaFromRgrItem(ErpItemUniqueCode $rgrItemUniqueCode, string $rcaType, string $bookParam, $authUser, $createJob = false) : array
    {
        $rgrSegregation = $rgrItemUniqueCode->segregation;
        if (!$rgrSegregation) {
            return ['status' => 'error', 'message' => 'RGR Item segregation not found'];
        }
        ...
        $pickupSchedule = null;
        if ($rgrHeader->pickup_schdule_id) {
            $pickupSchedule = ErpPickupSchedule::find($rgrHeader->pickup_schdule_id);
        }
        // RCA Header with RGR details
        $rcaHeader = ErpRcaHeader::create([
            ...
            'rgr_id' => $rgrHeader->id,
            'discrepancy_type' => $rcaType,
            'customer_id' => $rgrSegregation->rgrItem ?->customer_id ?? null,
            'customer_phone_no' => $rgrSegregation->rgrItem ?->customer->phone ?? null,
        ...
        $rcaHeader->document_status = !empty($approveDocument['approvalStatus']) 
         ? $approveDocument['approvalStatus'] : ConstantHelper::DRAFT;
        $rcaHeader->save();
