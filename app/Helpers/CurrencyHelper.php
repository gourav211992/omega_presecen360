...
class CurrencyHelper
{
    ...
    public static function getCurrencyExchangeRates(
        ...
        if (!$organization) {
            return self::errorResponse('Organization not mapped');
        }
        ...
        $partyCurrency = Currency::find($currencyId);
        if (!$partyCurrency) {
            return self::errorResponse('Invalid party currency');
        }

        // --- Organization Currency ---
        $organizationCurrency = Currency::find($organization->currency_id);
        if (!$organizationCurrency) {
            return self::errorResponse('Organization currency not mapped');
        }

        $organizationExchangeRate = $exchangeRate
            ?? ($currencyId === $organizationCurrency->id
                ? 1
                : self::getCurrencyExchangeRate(
                    $currencyId,
                    $organizationCurrency->id,
                    $transactionDate,
                    $group->id ?? 0,
                    $company->id ?? 0,
                    $organization->id
        ...
        if (!$organizationExchangeRate) {
            return self::errorResponse(
                "Exchange Rate not mapped for Organization {$partyCurrency->short_name} to {$organizationCurrency->short_name}"
            );
        }

        // --- Company Currency ---
        if (!$company) {
            return self::errorResponse('Company not mapped');
        }

        $companyCurrency = Currency::find($company->currency_id);
        if (!$companyCurrency) {
            return self::errorResponse('Company currency not mapped');
        }

        $companyExchangeRate = ($organizationCurrency->id === $companyCurrency->id)
            ? 1
            : self::getCurrencyExchangeRate(
                $organizationCurrency->id,
                $companyCurrency->id,
        ...
        if (!$companyExchangeRate) {
            return self::errorResponse(
                "Exchange Rate not mapped for Company {$organizationCurrency->short_name} to {$companyCurrency->short_name}"
            );
        }

        // --- Group Currency ---
        if (!$group) {
            return self::errorResponse('Group not mapped');
        }

        $groupCurrency = Currency::find($group->currency_id);
        if (!$groupCurrency) {
            return self::errorResponse('Group currency not mapped');
        }

        $groupExchangeRate = ($companyCurrency->id === $groupCurrency->id)
            ? 1
            : self::getCurrencyExchangeRate(
                $companyCurrency->id,
                $groupCurrency->id,
        ...
        if (!$groupExchangeRate) {
            return self::errorResponse(
                "Exchange Rate not mapped for Group {$companyCurrency->short_name} to {$groupCurrency->short_name}"
            );
        }
