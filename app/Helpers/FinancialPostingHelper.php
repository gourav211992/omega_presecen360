...
use App\Helpers\FinancialPosting\Sales\DeliveryNoteCumInvoicePosting;
use App\Helpers\FinancialPosting\Sales\DeliveryNotePosting;
use App\Helpers\FinancialPosting\Sales\SaleInvoicePosting;
use App\Helpers\FinancialPosting\Sales\ServiceInvoicePosting;
use App\Models\Book;
...
use App\Models\ExpenseAllocation\Header as ExpAlcHeader;
use App\Models\ExpenseAllocation\Detail as ExpAlcDetail;
...
class FinancialPostingHelper
{
    ...
    const FREIGHT_ACCOUNT = 'Freight';
    const INSURANCE_ACCOUNT = 'Insurance';
    ...
    const ADVANCE_PAYMENT_VOUCHER_RECEIPT_POSTING_ACCOUNT = [
        self::Bank_ACCOUNT => self::DEBIT,
        self::CUSTOMER_ACCOUNT => self::CREDIT,
    ];
    ...
    const SERVICE_POSTING_MAPPING = [
        ...
        ConstantHelper::ADVANCE_PAYMENTS_SERVICE_ALIAS => self::ADVANCE_PAYMENT_VOUCHER_RECEIPT_POSTING_ACCOUNT,
        ConstantHelper::ADVANCE_RECEIPTS_SERVICE_ALIAS => self::ADVANCE_PAYMENT_VOUCHER_RECEIPT_POSTING_ACCOUNT,
    ...
    public static function financeVoucherPosting(int $bookId, int $documentId, string $type, bool $onApproval = false)
    {
        ...
        if ($type === "view") {
            ...
            else if ($serviceAlias === ConstantHelper::ADVANCE_PAYMENTS_SERVICE_ALIAS || $serviceAlias === ConstantHelper::ADVANCE_RECEIPTS_SERVICE_ALIAS)
                return self::getAdvancePaymentDocumentPostedVouchers($documentId, $service->alias);
            else
        ...
        if ($serviceAlias === ConstantHelper::SI_SERVICE_ALIAS) {
            $entries = SaleInvoicePosting::voucherDetails($documentId, $type);
            if (!$entries['status']) {
                return array(
                    'status' => false,
            ...
            $entries = ServiceInvoicePosting::voucherDetails($documentId, $type);
            if (!$entries['status']) {
                return array(
                    'status' => false,
                    'message' => $entries['message'],
            ...
            $entries = DeliveryNotePosting::voucherDetails($documentId, $type);
            if (!$entries['status']) {
                return array(
                    'status' => false,
                    'message' => $entries['message'],
            ...
            $entries = DeliveryNoteCumInvoicePosting::voucherDetails($documentId, $type);
            if (!$entries['status']) {
                return array(
                    'status' => false,
                    'message' => $entries['message'],
        ...
        } else if ($serviceAlias === ConstantHelper::EXP_ALC_SERVICE_ALIAS) {
            $entries = self::expenseAllocationVoucherDetails($documentId, $type);
            if (!$entries['status']) {
                return array(
                    'status' => false,
                    'message' => $entries['message'],
        ...
        else if ($serviceAlias === ConstantHelper::ADVANCE_PAYMENTS_SERVICE_ALIAS) {

            $pay = self::advancepaymentInvoiceVoucherDetails($documentId, '');

            $entries = self::advancecontraVoucherDetails($documentId, '');
    ...
    public static function getAdvancePaymentDocumentPostedVouchers(int $documentId, string $serviceAlias)
    {
        $vouchers = Voucher::withoutGlobalScope(DefaultGroupCompanyOrgScope::class)->with(['ledger_items', 'series'])->where('reference_service', $serviceAlias)
            ->where('reference_doc_id', $documentId)->get();
    ...
    public static function pbVoucherDetails(int $documentId, string $type)
    {
        ...
        foreach ($document->items as $docItemKey => $docItem) {
            ...
            //SUPPLIER ACCOUNT
            $supplierCrAmount = ($itemValueAfterDiscount);
    ...
    public static function expenseAdviseVoucherDetails(int $documentId, string $type)
    {
        ...
        //COGS SETUP
        foreach ($document->items as $docItemKey => $docItem) {
            $itemValue = ($docItem->po_rate * $docItem->accepted_qty);
            $itemTotalDiscount = $docItem->header_discount_amount + $docItem->discount_amount;
            $itemValueAfterDiscount = $itemValue - $itemTotalDiscount;
            $stockDebitAmount = $discountSeperatePosting ? $itemValue : $itemValueAfterDiscount;
            $varianceAmount = $docItem->item_variance ?? 0.00;
    ...
    // Expense Allocation Voucher Details
    public static function expenseAllocationVoucherDetails(int $documentId, string $type)
    {
        $document = ExpAlcHeader::find($documentId);
        if (!isset($document)) {
            return array(
                'status' => false,
                'message' => 'Document not found',
        ...
        //SERVICE SETUP
        foreach ($document->poDetails as $docItemKey => $docItem) {
            $stockCreditAmount = $docItem->value;
            ...
            //Ledger found
            if (count($existingstockLedger) > 0) {
                $postingArray[self::SERVICE_ACCOUNT][0]['credit_amount'] += $stockCreditAmount;
            } else { //Assign a new ledger
                array_push($postingArray[self::SERVICE_ACCOUNT], [
                    'ledger_id' => $stockLedgerId,
                    'ledger_group_id' => $stockLedgerGroupId,
    ...
    public static function advancepaymentInvoiceVoucherDetails(int $documentId, string $remarks)
    {
        $accountSetup = isset(self::SERVICE_POSTING_MAPPING[ConstantHelper::ADVANCE_PAYMENTS_SERVICE_ALIAS]) ? self::SERVICE_POSTING_MAPPING[ConstantHelper::ADVANCE_PAYMENTS_SERVICE_ALIAS] : [];
        if (!isset($accountSetup) || count($accountSetup) == 0) {
            return array(
                'status' => false,
                'message' => 'Account Setup not found',
    ...
    public static function advancecontraVoucherDetails(int $documentId, string $remarks)
    {
        $organization = Helper::getAuthenticatedUser()->organization;
        $accountSetup = isset(self::SERVICE_POSTING_MAPPING[ConstantHelper::ADVANCE_PAYMENTS_SERVICE_ALIAS]) ? self::SERVICE_POSTING_MAPPING[ConstantHelper::ADVANCE_PAYMENTS_SERVICE_ALIAS] : [];
        if (!isset($accountSetup) || count($accountSetup) == 0) {
