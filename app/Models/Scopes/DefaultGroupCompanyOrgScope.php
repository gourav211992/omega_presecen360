...
class DefaultGroupCompanyOrgScope implements Scope
{
    ...
    public function apply(Builder $builder, Model $model)
    {
        if (property_exists($model, 'disableDefaultGroupCompanyOrgScope') &&
            $model->disableDefaultGroupCompanyOrgScope === true) {
            return;
        }

        if (isset($builder->withoutDefaultGroupCompanyOrgScope) &&
            $builder->withoutDefaultGroupCompanyOrgScope === true) {
            return;
        }

        $tableName = $model->getTable();
        $companyId = OrganizationHelper::getCompanyId();
        $groupId = OrganizationHelper::getGroupId();
        $organizationId = OrganizationHelper::getOrganizationId();

        $builder->where($tableName . '.group_id', $groupId)
            ->where(function ($q) use ($companyId, $tableName) {
                $q->whereNull($tableName . '.company_id')
                  ->orWhere($tableName . '.company_id', $companyId);
            })
