...
class ErpPslipItemHistory extends Model
{
    protected $table="erp_pslip_items_history";
    use HasFactory;
    protected $fillable = [
        'erp_scrap_id',
        'pslip_id',
    ...
    {
        return $this->belongsTo(ErpProductionSlipHistory::class, 'pslip_id', 'id');
    }
    ...
    {
        return $this->hasMany(PslipBomConsumptionHistory::class, 'pslip_item_id', 'id');
    }
    ...
    {
        return $this->belongsTo(ErpPslipItemAttributeHistory::class, 'pslip_item_id');
    }

    public function attributes()
    {
        return $this->hasMany(ErpPslipItemAttributeHistory::class, 'pslip_item_id');
    }
    ...
    public function item_attributes_array()
    {
        ...
        foreach ($itemAttributes as $attribute) {
            $existingAttribute = ErpPslipItemAttributeHistory::where('pslip_item_id', $this->getAttribute('id'))->where('item_attribute_id', $attribute->id)->first();
            ...
            foreach ($attribute_ids as $attributeValue) {
                ...
                if (isset($attributeValueData)) {
                    $isSelected = ErpPslipItemAttributeHistory::where('pslip_item_id', $this->getAttribute('id'))->where('item_attribute_id', $attribute->id)->where('attribute_value', $attributeValueData->value)->first();
    ...
    {
        return $this->hasMany(ErpPslipItemLocationHistory::class, 'pslip_item_id', 'id');
    }
    ...
    {
        return $this->hasMany(ErpPslipItemDetailHistory::class, 'pslip_item_id');
    }
    ...
    public function renderItemAttributesUI(int $rowIndex, int $maxCharLimit = 15): string
    {
        ...
        $pslipAttrs = ErpPslipItemAttributeHistory::where('pslip_item_id', $this->getAttribute('id'))
